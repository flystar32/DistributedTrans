...
public class SchedulerRollback {
    public static void main(String[] args) {
        SimpleTransferImpl simpleTransfer = GuiceConfig.getInstance().getInjector().getInstance(SimpleTransferImpl.class);

        simpleTransfer.scheduleRollback(10000);//超过10秒仍处于初始状态认为事务已经失败
    }
}
